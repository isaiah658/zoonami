...
function biome.background(mt_player_obj)
	...
	if biome[biome_name] then
		...
		local node_in_player = minetest.get_node(player_pos)
		player_pos.y = player_pos.y - 0.5
		local node_under_player = minetest.get_node(player_pos)
		if string.find(node_in_player.name, "water") or string.find(node_under_player.name, "water") then
			return "zoonami_underwater_background.png"
		elseif string.find(node_under_player.name, "grass") and biome_data.humidty and biome_data.humidty < 70 then
			return "zoonami_grassland_background.png"
		elseif string.find(node_under_player.name, "grass") and biome_data.humidty and biome_data.humidty >= 70 then
			return "zoonami_forest_background.png"
		elseif string.find(node_under_player.name, "grass") then
			return "zoonami_grassland_background.png"
		elseif string.find(node_under_player.name, "snow") then
			return "zoonami_snowy_background.png"
		elseif string.find(node_under_player.name, "sand") and biome_data.humidty and biome_data.humidty < 40 then
			return "zoonami_desert_background.png"
		elseif string.find(node_under_player.name, "sand") and biome_data.humidty and biome_data.humidty > 40 then
			return "zoonami_beach_background.png"
		elseif string.find(node_under_player.name, "sand") then
			return "zoonami_desert_background.png"
		elseif string.find(node_under_player.name, "stone") and player_pos.y <= -10 then
